...
crop_location= "/Users/marcus/Desktop/Training_Data/Crops_From_4096_2048/"
pano_location = "/Users/marcus/Desktop/Training_Data/Pano_4096_2048/"
...
def create_json_file_with_all_labels():
    for json_file in crop_json_list:
        ...
        pano_json_location = os.path.join(pano_location, data[0] + pano_width + pano_height + "_label1" + data[4])
        ...
        for x in crop['Pano_Data']:
            label_id = x['label_id']
        # Creates new json file incase one doesn't exist
        if not os.path.exists(pano_json_location):
            filedata = {}
            filedata[data[0]] = []
            open(pano_json_location, 'w')
        #Adds all the data
